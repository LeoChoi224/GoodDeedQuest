  ...
  return (
    <GamePopup visible={visible} onClose={onClose} title="참여 지역 선택" width={320}>
      <View style={{ alignSelf: 'stretch' }}>
        <Pressable style={styles.selRow} onPress={() => setSidoOpen(true)}>
          <Text style={styles.selText}>{sido}</Text>
          <ChevronDown />
        </Pressable>
        <Pressable style={styles.selRow} onPress={() => setSigunguOpen(true)}>
      ...
      {/* 시/도 드롭다운 */}
      <LightPopup visible={sidoOpen} onClose={() => setSidoOpen(false)} width={280}>
      ...
      {/* 시군구 드롭다운 */}
      <LightPopup visible={sigunguOpen} onClose={() => setSigunguOpen(false)} width={280}>
        <View style={styles.dropWrap}>
          {(SIGUNGU_MAP[sido] ?? DEFAULT_SIGUNGU).map((sg) => (
            <Pressable
              key={sg}
              style={styles.dropItem}
              onPress={() => {
...
const styles = StyleSheet.create({
  ...
  /* 지역 드롭다운 (LightPopup) */
  dropWrap: { alignSelf: 'stretch', width: '100%' },
  dropItem: { paddingVertical: 12, paddingHorizontal: 16, borderBottomWidth: 1, borderBottomColor: colors.divider },
  dropText: { fontSize: 15, fontFamily: fonts.bodyR, color: colors.textPrimary },
  dropActive: { color: colors.gold, fontFamily: fonts.bodyB },
});
